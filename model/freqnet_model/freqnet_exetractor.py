...
class FreqNet(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4], num_classes=1, zero_init_residual=False):
        ...
        self.weight1 = nn.Parameter(torch.randn((64, 3, 1, 1)).to(device))
        self.bias1 = nn.Parameter(torch.randn((64,)).to(device))
        ...
        self.weight2 = nn.Parameter(torch.randn((64, 64, 1, 1)).to(device))
        self.bias2 = nn.Parameter(torch.randn((64,)).to(device))
        ...
        self.weight3 = nn.Parameter(torch.randn((256, 256, 1, 1)).to(device))
        self.bias3 = nn.Parameter(torch.randn((256,)).to(device))
        ...
        self.weight4 = nn.Parameter(torch.randn((256, 256, 1, 1)).to(device))
        self.bias4 = nn.Parameter(torch.randn((256,)).to(device))
...
#     checkpoint = torch.load("../checkpoints/4-classes-freqnet-v2.pth", map_location="cpu")
...
#         # 过滤掉 fc1 层
#         if not k.startswith("fc1"):  # 这里是 fc1，不是 fc！
#             filtered_state_dict[k] = v
#
#     # ----------------------
#     # 3. 保存【只含特征提取层】的新权重
#     # ----------------------
#     torch.save(filtered_state_dict, "freqnet_backbone_only.pth")
#
#     print("✅ 处理完成！已删除 fc1 层，仅保存特征提取层！")
#     print("新文件：freqnet_backbone_only.pth")
